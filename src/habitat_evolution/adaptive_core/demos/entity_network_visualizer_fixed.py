...
class EntityNetworkVisualizer:
    ...
    def visualize_central_entities(self, filename: str = "central_entities.png", top_n: int = 20):
        ...
        # For betweenness centrality, adjust k for smaller networks
        node_count = len(self.network.nodes())
        k = min(node_count, 100) if node_count > 0 else 1
        
        # Only calculate betweenness centrality if we have enough nodes
        if node_count > 1:
            betweenness_centrality = nx.betweenness_centrality(self.network, k=k)
        else:
        ...
        # Get top N central entities (or all if fewer than top_n)
        actual_top_n = min(top_n, node_count)
        top_entities = sorted(combined_centrality.items(), key=lambda x: x[1], reverse=True)[:actual_top_n]
        ...
        # Draw nodes with unknown category
        unknown_nodes = [n for n in central_nodes if not self.network.nodes[n].get('category')]
        if unknown_nodes:
            nx.draw_networkx_nodes(
                central_graph, 
                pos, 
                nodelist=unknown_nodes,
                node_color='gray',
                node_size=[combined_centrality[n] * 5000 for n in unknown_nodes],
                alpha=0.8,
                label='Unknown'
            )
        ...
        plt.title(f"Top {actual_top_n} Central Entities in the Network")
        plt.legend(scatterpoints=1, loc='upper right')
        plt.axis('off')
        plt.tight_layout()
