...
async def test_pattern_graph_visualization(mock_patterns, request):
    ...
    # Convert nodes to PatternAdaptiveIDs
    pattern_ids = []
    id_mapping = {}
    for node in pattern_nodes:
        pattern_id = visualizer.create_pattern_id(
            pattern_type=node['type'],
            hazard_type=node['embedded_data']['hazard_type'],
            position=node['position'],
            field_state=node['embedded_data']['field_state'],
            coherence=node['metrics']['coherence'],
            energy_state=node['metrics']['energy_state']
        )
        id_mapping[node['id']] = pattern_id
        pattern_ids.append(pattern_id)
    
    # Add bidirectional relationships
    for edge in pattern_edges:
        source_pattern = id_mapping[edge['source']]
        target_pattern = id_mapping[edge['target']]
        ...
        target_pattern.add_relationship(
            target_id=source_pattern.id,
            relationship_type=edge['type'],
            metrics=edge['metrics']
        )
    
    # Visualize the graph representation
    fig = visualizer.visualize_pattern_graph(pattern_ids, field)
    ...
    visualizer._connect_to_neo4j()
    if visualizer._neo4j_driver:
        with visualizer._neo4j_driver.session() as session:
            session.run("MATCH (n) DETACH DELETE n")
    else:
        pytest.skip("Neo4j connection failed")
    
    # Export to Neo4j for persistence and analysis
    visualizer.export_pattern_graph_to_neo4j(pattern_ids, field)
    
    # Verify graph structure
    assert len(pattern_ids) == len(mock_patterns)
    assert all(hasattr(p, 'id') and hasattr(p, 'relationships') for p in pattern_ids)
    ...
    with visualizer._neo4j_driver.session() as session:
        ...
        """)
        assert result.single()['pattern_count'] == len(pattern_nodes)
        ...
        """)
        assert result.single()['field_count'] == 1
        
        # Check pattern-field connections
        result = session.run("""
            MATCH (p:Pattern), (f:FieldState)
            RETURN count(p) as connected_count
        """)
        assert result.single()['connected_count'] == len(pattern_nodes)
