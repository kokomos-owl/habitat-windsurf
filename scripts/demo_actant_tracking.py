...
def generate_metrics_table():
    """Generate and display an accumulative metrics table."""
    print("\n===== ACCUMULATIVE METRICS TABLE =====")
    ...
    try:
        # Use the list() method from the repositories to get all entities
        # Get all documents
        documents_list = document_repo.list()
        
        # Get all domains
        domains_list = domain_repo.list()
        
        # Get all predicates
        predicates_list = predicate_repo.list()
        
        # Get all actants
        actants_list = actant_repo.list()
        
        # Get resonance data
        resonances = tracker.find_resonating_domains(threshold=0.3)
        
        # Calculate actant metrics
        actant_domain_counts = {}
        actant_role_changes = {}
        actant_predicate_counts = {}
        
        for actant in actants_list:
            # Get actant journey
            journey = tracker.track_actant_journey(actant.name)
            
            # Count unique domains
            unique_domains = set()
            for step in journey:
                if "domain" in step and "_key" in step["domain"]:
                    unique_domains.add(step["domain"]["_key"])
            ...
            # Store metrics
            actant_domain_counts[actant.name] = len(unique_domains)
            actant_role_changes[actant.name] = role_changes
            actant_predicate_counts[actant.name] = len(journey)
        
        # Calculate domain metrics
        domain_actant_counts = {}
        domain_predicate_counts = {}
        
        for domain in domains_list:
            # Get predicates in domain
            domain_predicates = predicate_repo.find_by_domain_id(domain.id)
            domain_predicate_counts[domain.id] = len(domain_predicates)
            
            # Count unique actants in domain
            unique_actants = set()
            for predicate in domain_predicates:
                # Find actants for this predicate
                subject_actant = actant_repo.find_by_name(predicate.subject)
                object_actant = actant_repo.find_by_name(predicate.object)
                
                # Add actants to the set if they exist
                if subject_actant:
                    unique_actants.add(subject_actant.name)
                if object_actant:
                    unique_actants.add(object_actant.name)
            
            domain_actant_counts[domain.id] = len(unique_actants)
        ...
        print("-" * 50)
        print(f"{'Documents':<30} {len(documents_list):<10}")
        print(f"{'Domains':<30} {len(domains_list):<10}")
        print(f"{'Predicates':<30} {len(predicates_list):<10}")
        print(f"{'Actants':<30} {len(actants_list):<10}")
        print(f"{'Resonating Domain Pairs':<30} {len(resonances):<10}")
        
        # Display actant metrics
        print("\n2. ACTANT METRICS")
        print("-" * 80)
        print(f"{'Actant':<20} {'Domains':<10} {'Predicates':<10} {'Role Changes':<15} {'Boundary Crosser':<15}")
        print("-" * 80)
        
        for actant in actants_list:
            domains_count = actant_domain_counts.get(actant.name, 0)
            predicates_count = actant_predicate_counts.get(actant.name, 0)
            roles_count = actant_role_changes.get(actant.name, 0)
            boundary_crosser = "Yes" if domains_count > 1 else "No"
            
            print(f"{actant.name:<20} {domains_count:<10} {predicates_count:<10} {roles_count:<15} {boundary_crosser:<15}")
        
        # Display domain metrics
        print("\n3. DOMAIN METRICS")
        print("-" * 70)
        print(f"{'Domain ID':<36} {'Actants':<10} {'Predicates':<10} {'Resonances':<10}")
        print("-" * 70)
        
        # Count resonances per domain
        domain_resonance_counts = {}
        for resonance in resonances:
            domain1_id = resonance["domain1"]["_key"]
            domain2_id = resonance["domain2"]["_key"]
            
            if domain1_id not in domain_resonance_counts:
                domain_resonance_counts[domain1_id] = 0
            if domain2_id not in domain_resonance_counts:
                domain_resonance_counts[domain2_id] = 0
                
            domain_resonance_counts[domain1_id] += 1
            domain_resonance_counts[domain2_id] += 1
        
        for domain in domains_list:
            actants_count = domain_actant_counts.get(domain.id, 0)
            predicates_count = domain_predicate_counts.get(domain.id, 0)
            resonances_count = domain_resonance_counts.get(domain.id, 0)
            
            print(f"{domain.id:<36} {actants_count:<10} {predicates_count:<10} {resonances_count:<10}")
        
        # Display resonance summary
        print("\n4. RESONANCE SUMMARY")
        print("-" * 70)
        print(f"{'Threshold':<15} {'Resonance Count':<20} {'Avg. Shared Actants':<20}")
        print("-" * 70)
        
        # Calculate resonance at different thresholds
        for threshold in [0.3, 0.5, 0.7, 0.9]:
            resonances = tracker.find_resonating_domains(threshold=threshold)
            
            # Calculate average shared actants
            total_shared = sum(len(r["shared_actants"]) for r in resonances)
            avg_shared = total_shared / len(resonances) if resonances else 0
            
            print(f"{threshold:<15} {len(resonances):<20} {avg_shared:.2f}")
